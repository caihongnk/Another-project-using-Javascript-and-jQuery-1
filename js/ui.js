...
//--------------------Block区切换------------------------
$.fn.BlocksDisplay=function(){
	var ui=$(this);
	var blockContentA=$('#blockA .block-content',ui);
	var blockContentB=$('#blockB .block-content',ui);
	var captionBtn=$('.caption a',ui);
	var subBtnA=$('#blockA .block-caption a',ui);
	var subBtnB=$('#blockB .block-caption a',ui);
	var subPic=$('.sub-content .sub-content-pic img',ui);
	var location=$('.sub-content .location a',ui);
	var address=$('.sub-content .address a',ui);
             var map=$('.sub-content iframe',ui);

	captionBtn.on("click",function(){  
                  var index=$(this).index()
	     $('#blockA .sub-content').hide();     
	     $('#blockB .sub-content').hide();     
                  $(this).addClass("item_focus").siblings().removeClass("item_focus") 
                  
                  if(index===0){ $('#blockA').removeClass("displayNone"); $('#blockB').addClass("displayNone"); blockContentA.show(); subBtnA.eq(0).addClass("block-caption-item_focus").siblings().removeClass("block-caption-item_focus") }
                  if(index===1){ $('#blockB').removeClass("displayNone"); $('#blockA').addClass("displayNone"); blockContentB.show(); subBtnB.eq(0).addClass("block-caption-item_focus").siblings().removeClass("block-caption-item_focus") }
                  return false;
	})
          subBtnA.on("click",function(){
          	       $('.block-content').hide();
          	       $('#blockB .sub-content').hide(); 
                    $('#blockA .sub-content').show(); 
          	       $(this).addClass("block-caption-item_focus").siblings().removeClass("block-caption-item_focus")
                  var index=$(this).index()
                  if(index!=0){
                  subPic.attr("src","img/building-"+subA[index-1].src)
                  location.text(subA[index-1].locate)
                  address.text(subA[index-1].add)
                  }
                  if(index==0){ $('#blockA').removeClass("displayNone"); $('#blockB').addClass("displayNone"); blockContentA.show(); subBtnA.eq(0).addClass("block-caption-item_focus").siblings().removeClass("block-caption-item_focus")  }
          })      

          subBtnB.on("click",function(){ 
                  $('.block-content').hide();
          	     $('#blockA .sub-content').hide(); 
                  $('#blockB .sub-content').show(); 
          	     $(this).addClass("block-caption-item_focus").siblings().removeClass("block-caption-item_focus")
                  var index=$(this).index()
                  if(index!=0){
                  subPic.attr("src","img/building-"+subB[index-1].src)
                  location.text(subB[index-1].locate)
                  address.text(subB[index-1].add)
                  }
                  if( index==0){  $('#blockB').removeClass("displayNone"); $('#blockA').addClass("displayNone"); blockContentB.show(); subBtnB.eq(0).addClass("block-caption-item_focus").siblings().removeClass("block-caption-item_focus") }
          })  

}//$.fn.BlocksDisplay函数结束   
